...
export default function SupportPage() {
  const categories = [
    {
      ...
      icon: BookOpen,
      color: "text-[#B89868] dark:text-[#C5A880]",
      bg: "bg-[#B89868]/15 dark:bg-[#B89868]/20",
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F5] dark:bg-[#0E0F11] pb-20">
      {/* Search Hero Section */}
      <section className="relative py-20 lg:py-32 bg-slate-900 overflow-hidden">
        <div className="absolute inset-0 z-0">
           <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#B89868]/5 rounded-full blur-[120px]"></div>
           <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#C5A880]/5 rounded-full blur-[100px]"></div>
        </div>
        
        <div className="container relative z-10 mx-auto px-6 text-center space-y-8">
           <Badge className="bg-[#B89868]/10 text-[#B89868] dark:text-[#C5A880] border-[#B89868]/20 px-4 py-1.5 font-black">SUPPORT CENTER</Badge>
           ...
           <div className="max-w-2xl mx-auto relative pt-4 group">
             ...
             <div className="relative flex items-center">
                <Search className="h-6 w-6 absolute left-6 text-slate-500 group-focus-within:text-[#B89868] transition-colors" />
                <Input 
                  placeholder="Describe your issue (e.g., how to book a lawyer)" 
                  className="h-16 pl-16 pr-10 text-lg rounded-[2rem] bg-white/10 border-white/10 text-white placeholder:text-slate-500 focus:bg-white/20 focus:ring-[#B89868]/30 shadow-2xl transition-all"
                />
             </div>
           </div>
        </div>
      ...
      <section className="container mx-auto px-6 -mt-16 relative z-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {categories.map((cat, idx) => (
            <Card key={idx} className="rounded-[2.5rem] border-[#E5E2DC] dark:border-stone-800 bg-white dark:bg-[#0E0F11]/45 shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all group overflow-hidden">
                 ...
                 <ul className="space-y-4">
                    {cat.links.map((link, lidx) => (
                       <li key={lidx} className="flex items-center justify-between group/link cursor-pointer">
                          <span className="text-slate-600 dark:text-slate-400 font-medium group-hover/link:text-[#B89868] dark:group-hover/link:text-[#C5A880] transition-colors">{link}</span>
            ...
            <div className="lg:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-6">
               {[
                 ...
                 { q: "Do you operate in my country?", a: "We currently cover 18 African jurisdictions and are expanding rapidly across the continent." }
                  ...
                  <div key={idx} className="p-8 rounded-[2rem] bg-white dark:bg-stone-900/10 border border-[#E5E2DC] dark:border-stone-850 shadow-sm hover:shadow-md transition-shadow group">
                     <div className="h-10 w-10 rounded-xl bg-[#B89868]/10 dark:bg-[#B89868]/20 flex items-center justify-center mb-6">
                        <HelpCircle className="h-5 w-5 text-[#B89868] dark:text-[#C5A880]" />
                     </div>
                     <h4 className="text-lg font-black text-slate-900 dark:text-white mb-3 group-hover:text-[#B89868] transition-colors">{faq.q}</h4>
      ...
      <section className="container mx-auto px-6 pt-10">
         <div className="rounded-[3rem] bg-gradient-to-r from-[#B89868] to-[#C5A880] p-12 lg:p-20 text-center relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 max-w-2xl mx-auto space-y-8">
               ...
               <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Button size="lg" className="bg-white text-[#B89868] hover:bg-slate-50 px-10 h-16 rounded-2xl text-xl font-black shadow-xl shadow-black/10">
                     Chat to Support
                  </Button>
