      ...
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fade-in">
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 text-center max-w-sm w-full shadow-2xl border border-slate-200 dark:border-slate-800">
          ...
          <div className="flex flex-col gap-3">
            <Link href="/login">
              <button className="w-full py-4 bg-blue-600 dark:bg-teal-600 text-white font-bold rounded-2xl flex items-center justify-center gap-2 hover:bg-blue-700 dark:hover:bg-teal-700 transition-all">
                <LogIn size={20} />
                Sign In Now
              </button>
            </Link>
          ...
          {!isEligible ? (
             <div className="h-full flex flex-col items-center justify-center text-center py-10 space-y-6">
               ...
               <div className="space-y-2">
                 <h3 className="text-xl font-bold text-slate-900 dark:text-white">Professional Account Required</h3>
                 <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                   Consultation booking is currently limited to Citizens and NGO accounts.
                 </p>
               </div>
               <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-800 text-xs font-medium text-slate-500">
                 Your current role: <span className="text-blue-600 dark:text-teal-500 uppercase">{user?.role}</span>
               </div>
             </div>
          ) : (
